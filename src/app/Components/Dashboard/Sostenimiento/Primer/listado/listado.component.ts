...
export class ListadoComponent1 implements OnInit {
  ...
  cargarRegistros(): void {
    ...
    this.registroService.getPorEtapa(1).subscribe({
      next: (data) => {
        this.registros = data;
        this.loading = false;
      },
      error: (err) => {
        console.error('Error al cargar etapa 1:', err);
        this.loading = false;
      }
    });
  }
  ...
  verDetalle(item: RegistroLabor): void {
    this.router.navigate(
      ['/Dashboard/sostenimiento-formulario-1'],
      { state: { registro: item } }
    );
  }
}